...
<div class="navbar">
  <a href="librarian_profile.php">Profile</a>
	<a style="background: #ddd; color: black;" href="add_patron.php">Register Patron</a>
  <a href="add_librarian.php">Add Librarian</a>
  <a href="checkin.php">Check in</a>
  <a href="librarian_checkout.php">Check out</a>
...
$pname = $PassportNumber = $passsword = $passwordConfirm = $gender = $phonenumber = $address = $email = $city = $dob = $state =$image=$name= "";
if( isset($_GET["ID"])){
extract($_GET);
...
$PassportNumber = test_input($_POST["PassportNumber"]);
    ...
    if(strlen($pname)<5){
      $pnameErr = "must be between five character";
      $staus =false;
    }
  }
  ...
  if (empty($_POST["phoneNumber"])) {
    ...
    if(!preg_match('@[0-9]{11}@', $phonenumber)){
      $phoneNumberErr = "phone number must be eleven number";
        $staus =false;
    }
  }
...
if(empty($_POST["city"] )){
  $sql = " SELECT city FROM patron WHERE Passport_Number = '$PassportNumber'";
  ...
  $sql =  " INSERT INTO patron (city ) VALUES ('$city') WHERE Passport_Number = '$PassportNumber'";
  $result = mysqli_query($sql);
}

if(empty($_POST["state"] )){
  $sql = " SELECT state FROM patron WHERE Passport_Number = '$PassportNumber'";
  $quary = mysqli_query($conn, $sql);
  $row = mysqli_fetch_assoc($quary);
  if ($quary===false){
         echo "Error: " . $sql . "<br>" . mysqli_error($conn);
  ...
  $sql =  " INSERT INTO patron (state ) VALUES ('$state') WHERE Passport_Number = '$PassportNumber'";
  $result = mysqli_query($sql);
}
  if (empty($_POST["dob"])) {
  ...
  if (empty($_POST["PassportNumber"]) &&   $staus =false) {
    ...
    if ($quary===false){
          ...
          if ($resultCheck >1 ){
            header("Location: update_patron.php?error=IdAlreadyTaken");
            exit();
          } else {
            ...
            if( in_array($imageFileType,$extensions_arr) || !empty($name)){
                $image_base64 = base64_encode(file_get_contents($_FILES['file']['tmp_name']));
                $image = 'data:image/'.$imageFileType.';base64,'.$image_base64;
                $sql = $conn->query("UPDATE patron SET Patron_Name = '$pname' ,PASSWORD = '$password', gender = '$gender' , phone_number = '$phonenumber' , address = '$address', email ='$email' , city = '$city', state = '$state', dob = '$dob',
                image = '$image' , name = '$name' where 	Passport_Number= '$PassportNumber'");
                move_uploaded_file($_FILES['file']['tmp_name'],$target_dir.$name);
                  header("Location: view_patron.php?patron=updated");
         }
         if( empty($name)){
         $sql = "SELECT image , name from patron where Passport_Number='$PassportNumber'";
         $result = mysqli_query($conn, $sql);
         $row = mysqli_fetch_assoc($result);
         $name = $row['name'];
         $image = $row['image'];

           $sql = "UPDATE patron SET Patron_Name = '$pname' ,PASSWORD = '$password', gender = '$gender' , phone_number = '$phonenumber' , address = '$address', email ='$email' , city = '$city', state = '$state', dob = '$dob',
        name= '$name', image= '$image'  where 	Passport_Number= '$PassportNumber'";
            $result =mysqli_query($conn ,$sql);
              header("Location: view_patron.php?patron=updated");
...
<div class="container">
  ...
  <input type="text" placeholder="Enter your Passport number(i.e A123456789)" name="PassportNumber" maxlength="10" tabindex = 2 required value="<?php echo $PassportNumber; ?>">
  ...
  <input type="tel" placeholder="Enter phone number(09012312341)" name="phoneNumber" tabindex = 5 maxlength="11" required  maxlength="11" value="<?php echo $phonenumber; ?>">
  ...
  <hr>
  <button type="submit" style="width:150" tabindex = 12 class="registerbtn" name = "update">Update</button>
